...
//reset pwm generator
//time base = tmr2
void pwm_init(void) {						//set up the pwm generator
	//turn on pwm pin
#if defined(PWM_P1A)
	IO_OUT(PWM_DDR, PWM_P1A);
#endif

#if defined(PWM_P1B)
	IO_OUT(PWM_DDR, PWM_P1B);
#endif

#if defined(PWM_P1C)
	IO_OUT(PWM_DDR, PWM_P1C);
#endif

#if defined(PWM_P1D)
	IO_OUT(PWM_DDR, PWM_P1D);
#endif
